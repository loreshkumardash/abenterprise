...
class Ajax_pagination{
   ...
   var $show_count      = true;
	...
	function getAJAXlink( $count, $text) {
		...
		if(!empty($this->param_ar)){
			$param_str = "{'page' : $pageCount";
			foreach($this->param_ar as $k => $v){
				$param_str.= ",'$k' : '$v'"; 
			}
			$param_str .= "}";
	        $this->additional_param = $param_str;
		}else{
		...
		return "<a href=\"javascript:void(0);\"
		         " . $this->anchor_class . "
					onclick=\"$('.loading').show();$.post('". $this->base_url . $count ."', ". $this->additional_param .", function(data){
					$('". $this->target . "').html(data);" . $this->js_rebind .";$('.loading').hide(); }); return false;\">"
				. $text .'</a>';
	}
