...
function hoursAndMinutesToDays($hours, $minutes, $whr) {
    // Convert hours to minutes and add remaining minutes
    $totalMinutes = $hours * 60 + $minutes;

    // Calculate days
    $days = floor($totalMinutes / ($whr * 60));

    // Calculate remaining hours and minutes
    $remainingMinutes = $totalMinutes % ($whr * 60);
    $remainingHours = floor($remainingMinutes / 60);
    $remainingMinutes %= 60;

    return array($days, $remainingHours, $remainingMinutes);
}
...
function core_url($url){
    $url = str_replace('/erp', '', $url);
  return $url;
}

function assignedUserName($ids)
    {
        $CI = &get_instance();
       $as =  $CI->Common_Model->FetchData("users", "firstname, lastname", " userstatus = 1 AND user_id IN ($ids)");
       $name = [];
       $assigneName = "";
       if($as){

       for ($i=0; $i < count($as); $i++) { 
           $nm = $as[$i]['firstname'] . " " . $as[$i]['lastname'];
           $name[] = $nm;

       }
       $assigneName = implode(' , ' , $name);
    ...
    return ""; // Return empty string if URL is not recognized
}
