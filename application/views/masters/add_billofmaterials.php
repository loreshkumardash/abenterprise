...
<div class="wrapper">
        ...
        <div class="col-md-12">
          <div class="box">
            <div class="box-header with-border">
              <h3 class="box-title">New Bill of Material</h3>
            ...
            <div class="box-body">
            	<?php
	              ...
	              ?>
      				...
      				<?php
					    $select=$this->Common_Model->db_query("select *,LENGTH(id) from bill order by id DESC");
					    $row=$select[0];
					    $length=$row['LENGTH(id)'];
					    ...
					    else if($length=='1' AND $row['id']<='9'){                 
					       	$uname="GB0000";
					       	$id1=$row['id']+1;
					    	$username = $uname.$id1;  
					    }
					    else if($length=='2' AND $row['id']<='99'){
					     	$uname="GB000";
					       	$id1=$row['id']+1;
					       	$username = $uname.$id1; 
					    } 
					    else if($length=='3' AND $row['id']<='999'){
					     	$uname="GB00";
					       	$id1=$row['id']+1;
					       	$username = $uname.$id1; 
					    }  
					    else if($length=='4' AND $row['id']<='9999'){
					     	$uname="GB0";
					       	$id1=$row['id']+1;
					    	$username = $uname.$id1; 
					    }
					    else if($length=='5' AND $row['id']<='99999'){
					     	$uname="GB";
					       	$id1=$row['id']+1;
					       	$username = $uname.$id1; 
					    } 
					      ...
					      <div class="col-md-12 " >
					          <div class="nav-tabs-custom">
			                    <ul class="nav nav-tabs">
			                        <li  >
			                        	<a href="#bmm" data-toggle="tab">ADD BILL OF MATERIAL MASTER</a>
			                        </li>
			                        <li class="active">
			                        	<a href="#rmc" data-toggle="tab">RAW MATERIAL CONSUMED</a>
			                        </li>
			                    </ul>
					          <div class="tab-content" style="background-color:rgba(0,0,0,.03)!important;">
			                        

					          <div class="active tab-pane" id="rmc">
								...
								<div class="row " >
					  				<div class="col-md-12">
					    				<form action="" method="post"  enctype="multipart/form-data">
					    					<div class="tile">
					        					<div class="tile-body">
					     							<center><p style="padding: 10px;font-size: 20px;font-weight: 600;color: #f32f10 !important;text-transform: uppercase; ">RAW MATERIAL CONSUMED</p></center> 

  <?php 
  $select1=$this->Common_Model->db_query("select * from bill_raw where alias='$username' "); 
  if($select1){ ?> 
  ...
  <?php
  $tr="select * from bill_raw where alias='$username' order by display_order desc limit 1";
  ...
  <?php } ?>
					...
					$s1=$this->Common_Model->db_query("select * from `users` where user_id='$c_id1'");
					 ...
					 <input type="hidden" name="alias"  value="<?php echo $username; ?>" readonly>
								...
								<div class="tile">
								  <div class="tile-body">
								 	<div class="row">
								  		<div class="col-md-12">
								   		<?php
					            			$select=$this->Common_Model->db_query("select * from bill_raw  where alias='$username' and c_id='$c_id1' order by display_order ASC");
					          			if($select){
					              		?>
							              <div class="table-responsive">
							                <table class="table  table-bordered">
												...
												<tbody class="row_position">
												  <?php 
											      	...
											      	<tr id="<?php echo $row['id']?>"   draggable="true" ondragstart="start()"  ondragover="dragover()" style=" table-layout: fixed; width: 100%; border-collapse: collapse; border:1px solid #e7e7e7;">
											        	<td class="wib"  style="cursor: all-scroll;"> 
											          		<span id="sl"><?php echo $sl; ?></span>
											        	</td>
											         ...
											         <hr style="margin: 0px">
											            <?php echo $row['des'];?>
											          	...
											          	<td>
											            	<a href="<?=site_url();?>masters/billraweditt?id=<?php echo $row['id']; ?>" ><i class="fa fa-edit" style="color:green" title="edit"></i></a>
											              &nbsp;&nbsp;
											            	<a href="<?=site_url();?>masters/billrawdeletee?id=<?php echo $row['id']; ?>"><i class="fa fa-close" style="color: red" title="delete"></i></a></td>
											      	</tr> 
											       <?php } ?>
											    </tbody>
					              			</table>
					...
					<div class=" tab-pane" id="bmm"> 
					    <div class="tile">
					        <div class="tile-body">
								  <center><p style="padding: 10px;font-size: 20px;font-weight: 600;color: #f32f10 !important;text-transform: uppercase;"> Add Bill Of Material Master</p></center>
								...
								<div class="col-md-4">
								 	<div class="form-group">
										 ...
										 <input type="hidden" name="date" class="form-control" value="<?php echo date('Y-m-d') ?>" >  
										  <input type="text" name="bom" class="form-control" placeholder="Bill Of Material Name"  required >
								 	</div>
								</div>
								<div class="col-md-4">
								 	<div class="form-group">
								 		<label>ALIAS</label>
								 		<input type="text" name="alias" class="form-control" value="<?php echo $username; ?>" readonly>
								 	</div>
								</div>
							<div class="col-md-4">
								 <div class="form-group">
								 	<label>ITEM TO PRODUCE</label>
								 	<input list="item"  type="text" class="form-control" name="item" id="itemm">
								  ...
								  <div class="col-md-4">
								 	<div class="form-group">
										...
										<input type="number" pattern="[7-9]{1}[0-9]{9}" pattern="/^-?\d+\.?\d*$/" onKeyPress="if(this.value.length==10) return false;" name="qty" class="form-control" placeholder="Quantity" >
								 	</div>
								</div>
								  	<div class="col-md-4">
										<div class="form-group">
										 <label>Unit</label>
										 <input list="unit"  class="form-control" placeholder="Unit" type="text" name="unit" autocomplete="off">
							  				<datalist id="unit">
									     		<!-- <?php 
									            $sl=0;
								  ...
								  <div class="col-md-4">
								 	<div class="form-group">
										...
										<input type="text" name="exp_unit" placeholder="Expenses / Unit" class="form-control">
								 	</div>
						 		</div>
						 	</div>
								  <?php
...
<script type="text/javascript">
    ...
    $(document).on('change','#itemm',function(){
    	var code=$(this).val();
    	$('#unit').html('');
            $.ajax({
              type:'POST',
              url:'<?=site_url("masters/get_itemunits");?>',
              data:{code1:code},
              cache:false,
              dataType:"JSON",
              success:function(data)
              {
              	$('#unit').html('<option value="'+data.unit+'">'+data.unit+'</option><option value="'+data.alt_unit+'">'+data.alt_unit+'</option><option value="'+data.pack+'">'+data.pack+'</option>');
              }
            });
    });
