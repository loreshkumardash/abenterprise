        ...
        <div class="col-md-12">
          <div class="box">
            <div class="box-header with-border">
              <h3 class="box-title">Edit Unit Conversion</h3>
            ...
            <div class="box-body">
            	<?php
	              ...
	              ?>
              	<form method="post" action="<?=site_url("masters/edit_unitconversion/".$rec[0]['id']);?>">
              		<div class="col-md-4">
	                  <div class="row">
	                    ...
	                    <div class="col-md-12 form-group">
	                      ...
	                      <select name="main" id="main" class="form-control" required>
	                      	...
	                      	<?php if($unit){ for ($i=0; $i < count($unit); $i++) { ?>
	                      		<option value="<?=$unit[$i]['name'];?>" <?=$unit[$i]['name']==$rec[0]['main']?'selected':'';?>><?=$unit[$i]['name'];?></option>
	                      	<?php }} ?>
	                      </select>
	                    </div>
	                    <div class="col-md-12 form-group">
	                      ...
	                      <select name="sub" id="sub" class="form-control" required>
	                      	<option value="">Select</option>
	                      	<?php if($unit){ for ($i=0; $i < count($unit); $i++) { ?>
	                      		<option value="<?=$unit[$i]['name'];?>" <?=$unit[$i]['name']==$rec[0]['sub']?'selected':'';?>><?=$unit[$i]['name'];?></option>
	                      	<?php }} ?>
	                      </select>
	                    </div>
	                    <div class="col-md-12 form-group">
	                      <label for="factor">Con. Factor</label>
	                      <input type="number" name="factor" id="factor" class="form-control" placeholder="Enter con. factor" value="<?=$rec[0]['factor'];?>" required step="0.01">
	                    </div>
	                    <div class="col-md-12 form-group">
