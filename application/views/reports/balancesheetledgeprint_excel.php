                  <table class="table table-condensed table-striped" cellpadding="3">
                    <tr>
                      ...
                      <td width="30%" colspan="2"><b style="font-size:13px;">Group : <?=$group[0]['group_name'];?></b></td>
                    </tr>
                    <tr>
                      <td  width="70%" colspan="3"><span style="font-size:13px;"><b>Balance Sheet Details</b></span></td>
                      <td width="30%" colspan="2"><span style="font-size:13px;"><b><span><?php if($fdate){echo date('d M Y',strtotime($fdate)).' -' ; }?>  <?php if($ldate){ echo date('d M Y',strtotime($ldate)) ; }?></span></b></span></td>

                    </tr>
                  </table>
                  <table id="tblexportData" class="table  table-condensed table-striped" cellpadding="3" style="font-size: 12px;" border="1">
                    <thead>
                    ...
                    <tbody> 
                      ...
                      $ledg_creditasop = $this->Common_Model->db_query("SELECT SUM(cramount) as totcredit FROM entry_items as a LEFT JOIN entries as d on a.entry_id=d.entry_id LEFT JOIN ledgers as b on a.ledger=b.ledger_id LEFT JOIN under_group as c on b.acount_group=c.group_id WHERE  b.ledger_id=".$ledgers[$i]['ledger_id']." AND d.entry_dt < '".$fdate."'");
                        ...
                        <td><?=$dc;?> <?=number_format($opening,2);?></td>
