            ...
            <div class="box-body">
              <div class="row">
                ...
                <div class="col-md-6" style="padding-top: 15px;">
                  <button type="submit" formaction="<?php echo site_url("reports/transportfeestatus");?>" name="submitBtn" value="submit" class="btn bg-navy btn-flat margin">Search</button>
                  <button type="submit" formaction="<?php echo site_url("reports/download_transportfee_defaulter");?>" name="submitBtn1" value="submit" class="btn bg-navy btn-flat margin">Download Defaulter</button>
                  <button type="submit" formaction="<?php echo site_url("reports/download_transportfee_list");?>" name="submitBtn2" value="submit" class="btn bg-navy btn-flat margin">Download All</button>
                </div>
                </form>
              </div>
              <div class="row">
                <div class="col-md-12" id="dataTablediv">
                  ...
                  <table class="table table-bordered table-condensed table-striped">
                    ...
                    for($i=0;$i<count($students);$i++){
                      ...
                      $exists = db_query("SELECT * FROM student_transportfee WHERE student_id = ".$students[$i]['student_id']." AND admission_id = ".$students[$i]['admission_id']);
                      if($exists){
                        ?>
                    <tr>
                      ...
                      if($months){
                        foreach ($months as $month) {
                          $feelist = db_query("SELECT * FROM student_transportfee WHERE month_year = '".$month['monthyear']."' AND  student_id = ".$students[$i]['student_id']." AND admission_id = ".$students[$i]['admission_id']);
                          if($feelist){
                            $tdclass = '';
                            $amount = 0;
                            $status = '';
