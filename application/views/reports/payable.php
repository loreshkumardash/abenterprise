      ...
      <div class="row">
        <div class="col-md-12">
          <div class="box">
            ...
            <div class="box-body">
              <div class="row">
                <form role="form" action="<?php echo site_url("reports/payable");?>" method="post" id="searchForm">
                    <div class="col-md-2">
                  ...
                  <table id="tblexportData" class="table  table-condensed table-striped">
                    <thead>
                      <tr>
                        ...
                        <th width="15%" class="text-right">DUE ON<br>&nbsp</th>
                        <th width="10%" class="text-right">OVERDUE<br>BY DAYS</th>
                      </tr>
                    </thead>
                    <tbody>
                      <?php $totalpending = 0;
                      if ($purchase) {  for ($i=0; $i < count($purchase); $i++) {  

                          $start = strtotime($purchase[$i]['purchase_date']); $end = strtotime(date('Y-m-d')); $days_between = ceil(abs($end - $start) / 86400);

                          $paid = $this->Common_Model->db_query("SELECT SUM(itm_amount) as paid_amt FROM billadj_data WHERE invoice_id = ".$purchase[$i]['purchase_id']." AND adj_type='Payment'");
                          $due = $purchase[$i]['grand_total'] - $paid[0]['paid_amt'];

                          if ($due > 0) {   ?>
                      <tr>
                        <td width="10%" style="color:black;font-weight: 600;"><?=$purchase[$i]['purchase_date'];?></td>
                        <td width="15%" ><?=$purchase[$i]['invoice_no'];?></td>
                        <td width="25%" style="color:black;font-weight: 600;"><?=$purchase[$i]['ledger_name'];?></td>
                        <td width="10%" class="text-right" style="color:black;font-weight: 600;"><?=price_format(round($purchase[$i]['grand_total'],2));?></td>
                        <td width="10%" class="text-right" style="color:black;font-weight: 600;"><?=price_format(round($paid[0]['paid_amt'],2));?></td>
                        <td width="10%" class="text-right" style="color:black;font-weight: 600;"><?=price_format(round($due,2));?></td>
                        <td width="10%" class="text-right"><?=$purchase[$i]['purchase_date'];?></td>
