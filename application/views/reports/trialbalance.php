              ...
              <div class="row">
                  ...
                  <div class="col-md-3">
                    <div class="float-right">
                      <a href="<?=site_url('reports/trialbalance_pdf/'.$fdate.'/'.$ldate);?>" class="btn btn-primary btn-sm" style="width: 9rem;" target="_blank">Print as Pdf</a>
                      <a href="<?=site_url('reports/trialbalance_excel/'.$fdate.'/'.$ldate);?>" class="btn btn-warning btn-sm" style="width: 9rem;">Print as Excel</a>
                    </div>
                  </div>
                      ...
                      if ($subcategory) { for ($b=0; $b < count($subcategory); $b++) { 
                          ...
                          $subcat_debitasop = $this->Common_Model->db_query("SELECT SUM(dramount) as totdebit FROM entry_items as a LEFT JOIN entries as d on a.entry_id=d.entry_id LEFT JOIN ledgers as b on a.ledger=b.ledger_id LEFT JOIN under_group as c on b.acount_group=c.group_id WHERE  c.subcategory_id=".$subcategory[$b]['subcategory_id']." AND d.entry_dt < '".$fdate."'");
                            ...
                            <tr class="product_tr">
                              ...
                              <th><?php echo $dc.' '.number_format(abs($totopening),2); 
...
<script type="text/javascript">
 ...
 $(document).on('click', '.trialbalance_ledgers', function(){
      var id = $(this).attr('data-id');
      var fdate = $(".f_date").val();
      var ldate = $(".l_date").val();
      var url = "<?php echo site_url(); ?>";
      /*document.location.href=url+"/reports/todaysgrossprofitWithDay/"+id;*/
      document.location.href=url+'/reports/trialbalance_ledgers/'+id+'/'+fdate+'/'+ldate;
    });
    $(document).on('click', '#searchbtn', function(){
      ...
      var url = "<?php echo site_url(); ?>";
      /*document.location.href=url+"/reports/todaysgrossprofitWithDay/"+id;*/
      document.location.href=url+'/reports/trialbalance/'+fromdate+'/'+todate;
    }
    });
</script>
</body>
