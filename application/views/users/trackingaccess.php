          ...
          <div class="box">
            ...
            <div class="box-body">
              <div class="row">
                ...
                <form method="post" action="<?=site_url("users/trackingaccess");?>" >
                  <div class="col-md-12 row">
                    ...
                    <div class="col-md-2" style="margin-top:25px;">
                       <button type="button" name="downloadBtn" value="submit" id="downloadBtn" class="btn btn-default">Print Assign Report</button>
                    </div>
                    <input type="hidden" name="access_tracking_id" id="access_tracking_id">
...
<script type="text/javascript">
    ...
    $(document).on('change', '#user_id', function(){
        ...
        if (user_id) {
          $.ajax({
              url: '<?=site_url("users/get_trackingAccess");?>',
              data: {user_id : user_id},
              dataType:"HTML",
              type:"POST",
              success:function(data){
              if(data){
                $("#trackingdata").html(data.split('@#,')[1]);
                $("#access_tracking_id").val(data.split('@#,')[0]);
                
                    
                  
              }else{
                 $("#trackingdata").html('');
                 $("#access_tracking_id").val('');
              }
          }
          });
          } 
    ...
    $(document).on('click','#downloadBtn',function(){
        var user_id = $("#user_id").val();
        if (user_id > 0) {
        var url = '<?=site_url("users/trackingaccess");?>';
        window.open(url+'?user_id='+user_id, '_blank');
      }
    });
</script>
